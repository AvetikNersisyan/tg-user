export const ErrorCodes = {
  TG_ID_REQ: "Telegram id is required",
  DB_ERROR: "Db Error",
  INVALID_CHAT_ID: "INVALID_CHAT_ID",
  INVALID_TG_ID: "This user does not exist in db",
  INVALID_COUNTRY_ID: "Country id is invalid",
  NAME_ID_REQ: "Chat name and telegram id is required.",
  NO_DATA_PROVIDED: "Send data.",
  BULK_IS_NOT_ARRAY: "bulk should be an array!",
  INVALID_OUR_CHAT_ID: "We don't have our chat in db. First add that chat.",
  INVALID_JOIN_STATUS: "Join status is invalid. joined: 1, left: 2",
};
